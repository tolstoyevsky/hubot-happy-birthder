...
module.exports = function (robot) {
    ...
    find_users_born_on_date = function (date, users) {
        var uid, matches, user;
        matches = [];
        for (uid in users || {}) {
    ...
    var general_birthday_announcement = function (robot, birthday_users, image_url) {
    ...
    if (BIRTHDAY_CRON_STRING) {
        schedule.scheduleJob(BIRTHDAY_CRON_STRING, function () {
            var birthday_users, i, idx, len, msg, user;
    ...
    if (ANNOUNCER_CRON_STRING) {
        schedule.scheduleJob(ANNOUNCER_CRON_STRING, function () {
            ...
            let after_an_interval, birthday_users, birthday_user, bday_usr_id;
            ...
            for (bday_usr_id = 0; bday_usr_id < birthday_users.length; bday_usr_id++) {
                ...
                var msg_text = MSG_BIRTHDAY_IN_A_WEEK + `<@${birthday_user.name}>: ` +
